...
export default function App() {
  ...
  return (
    <>
      ...
      <div className="relative z-10 min-h-screen text-white font-sans selection:bg-white/20">
        <div className="max-w-[1400px] mx-auto p-4 md:p-8 lg:p-10 flex flex-col h-screen overflow-y-auto scrollbar-hide">
          ...
          <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 shrink-0">
            ...
            {/* Right: Search + Google Link + Settings + Profile */}
            <div className="flex items-center gap-3 w-full md:w-auto">
              ...
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-full bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.06] transition-all text-xs text-white/50 hover:text-white/80"
                title={settings.googleLinked ? `Linked: ${settings.googleEmail}` : 'Link Google Account'}
              >
                ...
                <span className="hidden sm:inline">{settings.googleLinked ? 'Connected' : 'Connect'}</span>
              </button>
      ...
      {/* Reader Overlay */}
      <ReaderOverlay
        book={isReading ? readingBook : null}
